...
The geolocation feature only fires after the user grants browser permission. Coordinates go to `GET /api/locations/nearby`, which calls the Nominatim reverse geocoding API (OpenStreetMap) to resolve a suburb or city name — they are not stored.
...
- Geolocation uses Nominatim reverse geocoding to resolve the user's coordinates to a suburb or city name. The resolved name is passed directly to the weather API, so results depend on what Nominatim returns.
